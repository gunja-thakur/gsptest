...
<body class="sb-nav-fixed">
            ...
            <div class="col-xl-12 col-lg-12">
              ...
              <div class="card mb-4 mt-2">
                <div class="card-header">
                  ...
                  <a href="<?php echo base_url(); ?>Placement/Add_Details" class="m-1 btn btn-sm btn-outline-success pull_right "> <i class="fa fa-plus"></i> Add Placement Detail</a>
                  <?php } ?>
                ...
                <div class="card-body">
                  ...
                  <?php if ($this->session->userdata['user_role'] == 1 or $this->session->userdata['user_role'] == 2 or $this->session->userdata['user_role'] == 3 ) { ?>
                    ...
                    <?php include(APPPATH . 'views/placement/placement_list_ajax.php'); ?>
                    </div>
  ...
  <script>
    $(document).on('click', '.RemovePdetails', function() {

      if (confirm("Are you sure to Remove The Placement Details?")) {
        ...
        $.ajax({
          ...
          url: "<?php echo base_url(); ?>Placement/Remove_PlacementDetails",
          method: "POST",
          data: {
            "pd_id": pd_id
          },
          success: function(res) {
            ...
            if (res == 1) {
              alert("Placement Details Removed Successfully!!");
              location.reload();
            } else {
  ...
  <script>
    $(document).on('click', '.ModifyPdetails', function() {
      ...
      $.ajax({
        ...
        url: "<?php echo base_url(); ?>Placement/View_PlacementDetail",
        method: "POST",
        data: {
          "pd_id": pd_id
        },
...
<script>
    $(document).on('click', '.ViewVacancy', function() {
      ...
      $.ajax({
        ...
        url: "<?php echo base_url(); ?>Vacancy/View_Vacancy",
        method: "POST",
        data: {
...
<script>
    $(document).on('click', '#SubmitData', function(event) {
      ...
      //alert("dfjhkd");return false;
      var vacancy_id = $("#vacancy_id").val();
      var verification_yn = $("input[name='verification_yn']:checked").val();
      ...
      //alert(vacancy_id + "-" + vr_status );return false;

      //if(confirm("Are you sure you want to verify the Vacancy Details ?")){
      $.ajax({
        url: "<?php echo base_url(); ?>Vacancy/VacancyVerification",
        method: "POST",
        data: {

          "vacancy_id": vacancy_id,
          "verification_yn": verification_yn
        },
        success: function(res) {
         //alert(res);return false;
         if(res==1)
         {
          alert("Vacancy Successfully Verified!!"); location.reload();
         }
         else
         {
...
<script>
    $(document).on('click', '#RecordSearch', function() {
      ...
      //alert(company_name+""+vr_status+""+from_date);return false
      ...
      $.ajax({
        url: "<?php echo base_url(); ?>Vacancy/ViewList_Ajax",
        type: "POST",
        data: {
