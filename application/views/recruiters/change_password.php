<?php foreach ($user_data as $get_sd) ?>
<!DOCTYPE html>
<html lang="en">
		...
		$(document).ready(function() {
			...
			$("#change").click(function() {
				...
				if (old_password.length < 8) {
					$("#old_password").focus();
					$("#old_password").css('border', '2px solid #ec000069').focus();
					$("#old_password_error").text('Should be atleast 8 characters');
					return false;
				}
				...
				$.ajax({
					...
					url: "<?= base_url() ?>Recruiters/ChangePassword_Save",
					method: "post",
					data: $("#changeform").serialize() + "&change=1",
					success: function(resp) {
						if (resp == 1) {
							alert("Password Changed Successfuly");
							window.location = "<?php echo base_url(); ?>RecruiterLogout";
						} else if (resp == 0) {
							alert("Old Password is Wrong!!!!!");
							window.location = "<?php echo base_url(); ?>Recruiters/ChangePassword";
						}
					}
